...
// Bandera para modo de prueba
window.AppConfig = window.AppConfig || {};
if (!window.AppConfig.hasOwnProperty('TEST_MODE')) {
    window.AppConfig.TEST_MODE = false;
}
    ...
    firebasePromise.then(() => {
        ...
        async function loadUsers() {
            try {
                ...
                users = [];
                renderUsers();
                alert('No se pudieron cargar los usuarios debido a permisos insuficientes. Contacta al administrador.');
            }
        }
        ...
        async function applyPermissions() {
            ...
            if (!hasFormAccess) {
                ...
                document.querySelectorAll('.form-group').forEach(group => {
                    ...
                    if (!fieldKey) {
                        console.warn(`Elemento .form-group sin data-permission: ${group.outerHTML}`);
                        return;
                    }
                    if (!allowedFields.includes(fieldKey)) {
                        console.log(`Ocultando elemento ${fieldKey}`);
                        group.style.display = 'none';
                    } else {
                        console.log(`Mostrando elemento ${fieldKey}`);
                        group.style.display = ''; // Asegurar que el elemento sea visible
                    }
                });
            }
